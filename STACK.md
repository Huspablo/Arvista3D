...
**Tecnología elegida:**
- `Zustand` para estado UI/local del viewer
- `TanStack Query` para datos remotos y caché cliente

**Cómo se usa**
- Zustand gestiona estado del visor 3D y de interacción: obra seleccionada, modo exploración, overlays, cámara, filtros de sala.
- TanStack Query gestiona lecturas remotas, caché, refetch y sincronización de datos de galerías, obras y perfil.

**Por qué se elige**
- Separa claramente estado de interfaz de estado remoto.
- Evita sobrecargar React con estado cruzado innecesario.
- Escala bien en una app donde conviven dashboard, viewer y operaciones de contenido.

**Decisión práctica**
- No usar Redux para el MVP.
- Mantener el viewer desacoplado del resto de pantallas mediante un store ligero.
...
| Estado local | Zustand | Ligero y adecuado para el viewer |
